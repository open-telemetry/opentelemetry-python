...
def update_dependencies(targets, version, packages):
    ...
    for pkg in packages:
        # `[^,\n]*` keeps the match on a single line so a package's own static
        # `version = "...dev"` line (for Maturin packages) is not swallowed.
        search = rf"({basename(pkg)}[^,\n]*)({operators_pattern})(.*\.dev)"
...
def update_patch_dependencies(targets, version, prev_version, packages):
    ...
    for pkg in packages:
        search = rf"({basename(pkg)}[^,\n]*?)(\s?({operators_pattern})\s?)(.*{prev_version})"
