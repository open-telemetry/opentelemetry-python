...
class TestB3MultiTraceContextEnvPropagator(unittest.TestCase):
    def setUp(self):
        ...
        self.composite_env_propagator_w3c_b3_object = CompositeEnvPropagator([TraceContextTextMapPropagator(), B3MultiFormat()])
        self.composite_env_b3_w3c_baggage_propagator_object = CompositeEnvPropagator([B3MultiFormat(), TraceContextTextMapPropagator(), W3CBaggagePropagator()])
        self.composite_env_w3c_b3_baggage_propagator_object = CompositeEnvPropagator([TraceContextTextMapPropagator(), B3MultiFormat(), W3CBaggagePropagator()])
        self.composite_env_propagator_None_object = CompositeEnvPropagator(None)
        self.baggage_propagator_object = W3CBaggagePropagator()
    ...
    def test_inject_when_valid_context_with_baggage_attached_for_all_3_formats(self):
        ...
        token = opentelemetry.context.attach(dummy_context)
        traceid, spanid, sampled, parentspanid, tracestate = get_details_from_context(dummy_context)
        self.composite_env_b3_w3c_baggage_propagator_object.inject(carrier = self.mock_carrier, setter = DefaultSetter())
        opentelemetry.context.detach(token)
        self.assertEqual(self.mock_carrier.get("traceparent") , '00'+'-'+str(traceid)+'-'+str(spanid)+'-'+'0'+str(sampled))
        self.assertEqual(self.mock_carrier.get("baggage") , "key1=1,key2=2")
        self.assertEqual(self.mock_carrier.get("x-b3-traceid") , str(traceid))
    ...
    def test_inject_when_valid_context_with_baggage_passed_for_all_3_formats(self):
        dummy_context = create_valid_context_with_baggage()
        traceid, spanid, sampled, parentspanid, tracestate = get_details_from_context(dummy_context)
        self.composite_env_b3_w3c_baggage_propagator_object.inject(carrier = self.mock_carrier, context = dummy_context, setter = DefaultSetter())
        self.assertEqual(self.mock_carrier.get("traceparent") , '00'+'-'+str(traceid)+'-'+str(spanid)+'-'+'0'+str(sampled))
        self.assertEqual(self.mock_carrier.get("baggage") , "key1=1,key2=2")
        self.assertEqual(self.mock_carrier.get("x-b3-traceid") , str(traceid))
        self.assertEqual(self.mock_carrier.get("x-b3-spanid") , str(spanid))
    ...
    def test_inject_to_carrier_when_valid_context_is_attached_for_all_3_formats(self):
        ...
        token = opentelemetry.context.attach(dummy_context)
        traceid, spanid, sampled, parentspanid, tracestate = get_details_from_context(dummy_context)
        self.mock_carrier = self.composite_env_b3_w3c_baggage_propagator_object.inject_to_carrier()
        opentelemetry.context.detach(token)
        self.assertEqual(self.mock_carrier.get("traceparent") , '00'+'-'+str(traceid)+'-'+str(spanid)+'-'+'0'+str(sampled))
        self.assertEqual(self.mock_carrier.get("baggage") , "key1=1,key2=2")
        self.assertEqual(self.mock_carrier.get("x-b3-traceid") , str(traceid))
    ...
    def test_inject_to_carrier_when_valid_context_passed_as_param_for_all_3_formats(self):
        dummy_context = create_valid_context_with_baggage()
        traceid, spanid, sampled, parentspanid, tracestate = get_details_from_context(dummy_context)
        self.mock_carrier = self.composite_env_b3_w3c_baggage_propagator_object.inject_to_carrier(context = dummy_context)
        self.assertEqual(self.mock_carrier.get("traceparent") , '00'+'-'+str(traceid)+'-'+str(spanid)+'-'+'0'+str(sampled))
        self.assertEqual(self.mock_carrier.get("baggage") , "key1=1,key2=2")
        self.assertEqual(self.mock_carrier.get("x-b3-traceid") , str(traceid))
        self.assertEqual(self.mock_carrier.get("x-b3-spanid") , str(spanid))
    ...
    def test_extract_context_when_no_trace_details(self):
        extracted_context = self.composite_env_b3_w3c_baggage_propagator_object.extract_context()
    ...
    def test_extract_context_when_baggage_is_valid_and_traceid_of_w3c_and_b3_have_16_hex_char(self):
        ...
        extracted_context = self.composite_env_w3c_b3_baggage_propagator_object.extract_context()
        traceid, spanid, sampled, parentspanid, tracestate = get_details_from_context(extracted_context)
    ...
    def test_extract_context_when_w3c_and_b3_traceid_and_spanid_are_invalid(self):
        # order of propagators in composite propagator: TraceContextTextMapPropagator, B3
        extracted_context = self.composite_env_propagator_w3c_b3_object.extract_context()
        self.assertEqual(opentelemetry.trace.get_current_span(extracted_context), opentelemetry.trace.span.INVALID_SPAN)
    ...
    @mock.patch.dict(os.environ, {"x-b3-parentspanid": "e28bf981e15deb7f"})
    @mock.patch.dict(os.environ, {"traceparent": "00-8128c50f-136eec09c-01"})
    def test_extract_context_when_b3_and_w3c_traceid_and_spanid_are_invalid(self):
    ...
    def test_extract_when_no_trace_details(self):
        extracted_context = self.composite_env_b3_w3c_baggage_propagator_object.extract(getter = getter, carrier = os.environ)
        self.assertEqual(opentelemetry.trace.get_current_span(extracted_context), opentelemetry.trace.span.INVALID_SPAN)
    ...
    def test_extract_when_baggage_is_valid_and_traceid_of_b3_and_w3c_have_16_hex_char(self):
        ...
        def test_extract_when_baggage_is_valid_and_traceid_of_w3c_and_b3_have_16_hex_char(self):
            # order of propagators in composite propagator: TraceContextTextMapPropagator, B3
            extracted_context = self.composite_env_w3c_b3_baggage_propagator_object.extract(getter = getter, carrier = os.environ)
            traceid, spanid, sampled, parentspanid, tracestate = get_details_from_context(extracted_context)
            self.assertEqual(traceid , '000000000000000098fea4de58eca772')
            self.assertEqual(spanid , '136eec09c948be26')
            self.assertEqual(sampled , 1)
    ...
    def test_extract_when_w3c_and_b3_traceid_is_invalid(self):
        ...
        # current span is invalid as b3 returns invalid span in context
        self.assertEqual(opentelemetry.trace.get_current_span(extracted_context), opentelemetry.trace.span.INVALID_SPAN)
    ...
    @mock.patch.dict(os.environ, {"baggage": "example1-value1;example2-value2"})
    def test_extract_when_baggage_format_invalid_and_baggage_propagator_passed_to_composite_propagator(self):
        extracted_context = self.baggage_propagator_object.extract(getter= getter, carrier = os.environ)
