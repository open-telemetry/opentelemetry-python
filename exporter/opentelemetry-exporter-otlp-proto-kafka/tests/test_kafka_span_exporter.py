...
from opentelemetry.exporter.otlp.proto.kafka._internal import trace_exporter
...
class TestOTLPSpanExporter(unittest.TestCase):
    ...
    @patch.object(trace_exporter, "encode_spans")
    def test_export(self, mock_encode_spans):
        mock_spans = Mock()
        self.assertEqual(
            OTLPSpanExporter(producer=self.mock_producer).export(mock_spans),
            SpanExportResult.SUCCESS,
        )

        mock_encode_spans.assert_called_once_with(mock_spans)
        self.mock_producer.send.assert_called_once_with(
            "otlp_spans",
            value=mock_encode_spans.return_value.SerializePartialToString.return_value,
            headers=[],
        )
    ...
    @patch.dict(
        "os.environ",
        {OTEL_EXPORTER_KAFKA_TRACES_TOPIC: "traces_topic"},
    )
    @patch.object(trace_exporter, "encode_spans")
    def test_export_env_logs_topic(self, mock_encode_logs):
        ...
        self.mock_producer.send.assert_called_once_with(
            "traces_topic",
            value=mock_encode_logs.return_value.SerializePartialToString.return_value,
            headers=[],
        )
    ...
    def test_export_constructor_logs_topic(self, mock_encode_logs):
        ...
        self.assertEqual(
            OTLPSpanExporter(
                producer=self.mock_producer, topic="traces_topic"
            ).export(mock_spans),
            SpanExportResult.SUCCESS,
        )
    ...
    def test_shutdown(self):
        OTLPSpanExporter(producer=self.mock_producer).shutdown()

        self.mock_producer.close.assert_called_once()

    @patch.object(trace_exporter, "encode_spans")
    def test_export_kafka_error(self, mock_encode_spans):
        self.mock_producer.send.side_effect = InvalidRequiredAcksError()
        with self.assertLogs(level=ERROR) as error:
            self.assertEqual(
                OTLPSpanExporter(producer=self.mock_producer).export(Mock()),
                SpanExportResult.FAILURE,
            )
            self.assertIn(
                "Failed to export span batch reason:",
                error.records[0].message,
            )

    def test_export_post_shutdown_fails(self):
        exporter = OTLPSpanExporter(producer=self.mock_producer)
        exporter.shutdown()
    ...
    def test_force_flush(self):
        self.assertTrue(
            OTLPSpanExporter(producer=self.mock_producer).force_flush(),
        )
