...
from opentelemetry.exporter.otlp.proto.http_light.metric_exporter import (
...
# OTEL env vars that may be set in the developer's environment and would
# contaminate tests that check default/specific values.
_OTEL_METRICS_ENV_VARS = [
    OTEL_EXPORTER_OTLP_ENDPOINT,
    OTEL_EXPORTER_OTLP_HEADERS,
    OTEL_EXPORTER_OTLP_TIMEOUT,
    OTEL_EXPORTER_OTLP_COMPRESSION,
...
class TestOTLPMetricExporter(TestCase):
    ...
    def setUp(self):
        ...
        self._saved_env = {}
        for var in _OTEL_METRICS_ENV_VARS:
    ...
    def test_success(self, mock_post):
        mock_post.return_value = _OTLPHTTPResponse(200, "OK")
    ...
    def test_failure(self, mock_post):
        mock_post.return_value = _OTLPHTTPResponse(401, "Unauthorized")
    ...
    def test_serialization(self, mock_post):
        ...
        # light exporter calls self._client.post(data, timeout_sec) — positional args
        # patch.object binds self, so mock receives (data, timeout)
        mock_post.assert_called_once_with(serialized_data.SerializeToString(), ANY)
    ...
    def test_export_max_export_batch_size_single_batch_integration(
        ...
        # positional args: (data, timeout) — patch.object binds self
        call_args = mock_post.call_args
        batch_data = call_args[0][0]
        self.assertIsInstance(batch_data, bytes)
    ...
    def test_export_max_export_batch_size_multiple_batches_integration(
        ...
        # positional args: (data, timeout) — patch.object binds self
        first_batch_data = mock_post.call_args_list[0][0][0]
        ...
        second_batch_data = mock_post.call_args_list[1][0][0]
    ...
    def test_export_max_export_batch_size_retry_scenarios_integration(
        ...
        success_resp = _OTLPHTTPResponse(200, "OK")
        failure_resp = _OTLPHTTPResponse(400, "Bad Request")
    ...
    def test_export_max_export_batch_size_retryable_failure_integration(
        ...
        success_resp = _OTLPHTTPResponse(200, "OK")
        retryable_failure_resp = _OTLPHTTPResponse(503, "Service Unavailable")
        ...
        second_batch_data = mock_post.call_args_list[2][0][0]
        second_request = ExportMetricsServiceRequest()
        second_request.ParseFromString(second_batch_data)
        self.assertEqual(len(second_request.resource_metrics), 1)
        second_metrics = (
