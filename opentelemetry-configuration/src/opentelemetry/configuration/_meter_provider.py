...
def _create_otlp_file_development_metric_exporter(
    ...
    try:
        # pylint: disable=import-outside-toplevel,no-name-in-module
        from opentelemetry.exporter.otlp.json.file.metric_exporter import (  # type: ignore[import-untyped]
            FileMetricExporter,
        )
    except ImportError as exc:
        raise ConfigurationError(
...
def _create_prometheus_metric_reader(
    ...
    try:
        # pylint: disable=import-outside-toplevel,no-name-in-module
        from opentelemetry.exporter.prometheus import (  # type: ignore[import-untyped]
