...
follows:
    ...
    1. If the environment variable
       ...
       instance of a :class:`~opentelemetry.trace.Tracer` (probably of a derived
       type).
    2. Otherwise, ``OPENTELEMETRY_PYTHON_IMPLEMENTATION_DEFAULT`` is tried instead.
    3. Otherwise, if a :samp:`set_preferred_{<type>}_implementation` was called,
    ...
    6. Otherwise the default implementation that ships with the API distribution
       (a fast no-op implementation) is used.

If any of the above steps fails (e.g., a module is loaded but does not define
the required function or a module name is set but the module fails to load), the
search immediatelly skips to the last step.
"""
...
_DEFAULT_IMPLEMENTATION_MODNAME = 'opentelemetry.sdk.internal.implementation_impl'
...
def _try_load_impl_from_modname(
    ...
    return _try_load_impl_from_mod(True, implementation_mod, api_type)

_preferred_implementation = dict()

def _try_load_impl_from_mod(
        swallow_errors: bool, implementation_mod: object, api_type: Type[_T]) -> Optional[_T]:

    try:
        # Note: We use such a long name to avoid calling a function that is not intended for this
        # API.
        implementation_fn = getattr(implementation_mod, 'get_opentelemetry_implementation')
    ...
    if not isinstance(result, api_type):
        ...
        raise TypeError('Expected a {} but got a {}'.format(
                api_type.__qualname__, type(result).__qualname__))
    # TODO: Warn if implementation_fn returns api_type(): It should return None to indicate using the
    # default.
    return result
...
def _try_load_configured_impl(api_type: Type[_T]) -> Optional[_T]:
    ...
    if implementation_modname:
        return _try_load_impl_from_modname(api_type, implementation_modname)
    implementation_modname = os.getenv('OPENTELEMETRY_PYTHON_IMPLEMENTATION_DEFAULT')
    if implementation_modname:
        return _try_load_impl_from_modname(api_type, implementation_modname)
    if _preferred_implementation[api_type] is not None:
        return _try_load_impl_from_mod(True, _preferred_implementation, api_type)
    if _preferred_implementation[None] is not None:
        return _try_load_impl_from_mod(True, _preferred_implementation, api_type)
    return None
...
def set_preferred_default_implementation(implementation_module: object) -> None:
    """Sets a module that may be queried for a default implementation. See the
    module docs for more details."""
    _preferred_implementation[None] = implementation_module
