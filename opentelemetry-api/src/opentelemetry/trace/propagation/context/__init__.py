...
def from_context(ctx: BaseRuntimeContext) -> SpanContext:
    return BaseRuntimeContext.value(ctx, ContextKeys.span_context_key(),)


def with_span_context(
    ctx: BaseRuntimeContext, span_context: SpanContext
) -> BaseRuntimeContext:
    return BaseRuntimeContext.set_value(
