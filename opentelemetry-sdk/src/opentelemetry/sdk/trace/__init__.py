...
class Span(trace_api.Span, ReadableSpan):
    ...
    def record_exception(
        ...
        *,
        ...
        event_name = event_name if event_name is not None else "exception"
        severity_number = severity_number if severity_number is not None else SeverityNumber.ERROR
    ...
    def _record_exception_log(
        ...
        *,
        ...
        # Checked without `self._lock` on purpose; the lock would be held
        # across `emit()`, blocking `end()` for the duration of a synchronous
        # export, and a concurrently ending span is not worth that.
        if self._end_time is not None:
            logger.warning("Tried calling record_exception on an ended span.")
            return

        log_attributes: dict[str, types.AnyValue] = {
            EXCEPTION_TYPE: exception_type,
        ...
        exception_logger = _logs.get_logger(
            ...
            attributes=scope.attributes if scope else None,
        )
        # The `exception.escaped` attribute is intentionally omitted as it is
        # deprecated for the logs representation.
