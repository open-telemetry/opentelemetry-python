...
class SpanProcessorMetrics:
    def __init__(
        ...
        *,
        ...
        self._processed_spans = create_otel_sdk_processor_span_processed(meter)

        if capacity is not None:
            self._queue_capacity = (
                create_otel_sdk_processor_span_queue_capacity(meter)
            )
    ...
    def drop_items(self, count: int) -> None:
        self._processed_spans.add(count, self._dropped_attrs)

    def finish_items(self, count: int, error: Exception | None) -> None:
        if not error:
            self._processed_spans.add(count, self._standard_attrs)
            return
        attrs = {
            **self._standard_attrs,
            ERROR_TYPE: type(error).__name__,
        }
        self._processed_spans.add(count, attrs)
