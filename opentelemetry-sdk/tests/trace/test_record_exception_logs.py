...
class TestRecordExceptionSignalOptIn(unittest.TestCase):
    def setUp(self):
        ...
        self.tracer = provider.get_tracer("test-scope", "1.0", attributes={"scope.key": "v"})
    ...
    def test_unrecognized_value_records_span_event_only(self):
        with mock.patch.dict(os.environ, {OTEL_SEMCONV_EXCEPTION_SIGNAL_OPT_IN: "bogus"}):
            self._raise_in_span()
        self.assertEqual(len(self._exception_events()), 1)
        self.assertEqual(self.logger.records, [])

    def test_logs_records_log_only(self):
        with mock.patch.dict(os.environ, {OTEL_SEMCONV_EXCEPTION_SIGNAL_OPT_IN: "logs"}):
            self._raise_in_span()
        ...
        self.assertIn("ValueError: boom", record.attributes["exception.stacktrace"])
    ...
    def test_logs_dup_records_both(self):
        with mock.patch.dict(os.environ, {OTEL_SEMCONV_EXCEPTION_SIGNAL_OPT_IN: "logs/dup"}):
            self._raise_in_span()
        self.assertEqual(len(self._exception_events()), 1)
        self.assertEqual(len(self.logger.records), 1)
    ...
    def test_logs_uses_instrumentation_scope_for_logger(self):
        ...
        args, kwargs = self.mock_get_logger.call_args
        self.assertEqual(args[0], "test-scope")
        self.assertEqual(dict(kwargs["attributes"]), {"scope.key": "v"})

    def test_ended_span_does_not_emit_log(self):
        with mock.patch.dict(os.environ, {OTEL_SEMCONV_EXCEPTION_SIGNAL_OPT_IN: "logs"}):
    ...
    def test_event_name_and_severity_can_be_overridden(self):
        with mock.patch.dict(os.environ, {OTEL_SEMCONV_EXCEPTION_SIGNAL_OPT_IN: "logs/dup"}):
            with self.tracer.start_as_current_span("op") as span:
                span.record_exception(
                    ValueError("boom"),
                    event_name="http.client.request.exception",
                    severity_number=SeverityNumber.WARN,
                )
        record = self.logger.records[0]
        self.assertEqual(record.event_name, "http.client.request.exception")
        self.assertEqual(record.severity_number, SeverityNumber.WARN)
        self.assertEqual(record.severity_text, "WARN")
        # The span event representation always uses the "exception" name.
        self.assertEqual(len(self._exception_events()), 1)

    def test_extra_attributes_forwarded_to_log(self):
        with mock.patch.dict(os.environ, {OTEL_SEMCONV_EXCEPTION_SIGNAL_OPT_IN: "logs"}):
            with self.tracer.start_as_current_span("op") as span:
                span.record_exception(ValueError("boom"), attributes={"custom.key": "v"})
        record = self.logger.records[0]
