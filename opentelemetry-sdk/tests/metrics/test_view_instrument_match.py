...
class Test_ViewInstrumentMatch(TestCase):  # pylint: disable=invalid-name
    ...
    def test_attributes_advisory(self):
        for instrument_class in (
            ...
            for view_attribute_keys, advisory_attributes, expected_attributes in (
                (None, {"a"}, {"a": 1}),
                (None, None, {"a": 1, "b": 2}),
                (None, (), {}),
                ({"b"}, {"a"}, {"b": 2}),
                ...
                with self.subTest(
                    ...
                    instrument = instrument_class(
                        ...
                        _advisory=_MetricsAdvisory(
                            attributes=None if advisory_attributes is None else frozenset(advisory_attributes)
                        ),
                    )
                    view_instrument_match = _ViewInstrumentMatch(
                        view=View(
                    ...
                    self.assertEqual(
                        ...
                        [_hash_attributes(expected_attributes)],
                    )
